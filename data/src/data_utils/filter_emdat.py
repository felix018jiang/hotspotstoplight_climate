...
def filter_data_from_gcs(country_name):
    ...
    filtered_data = filtered_data.copy()  # Create a copy of the DataFrame to avoid modifying the original data
    ...
    date_pairs = [(start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d')) for start_date, end_date in zip(filtered_data['start_date'], filtered_data['end_date'])]
