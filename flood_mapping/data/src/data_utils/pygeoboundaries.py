...
def _is_valid_adm(iso3, adm: str) -> bool:
    ...
    html = session.get(f"https://www.geoboundaries.org/api/current/gbOpen/{iso3}/", verify=True).text
    return adm in html
...
def _generate_url(territory: str, adm: Union[str, int]) -> str:
    ...
    if not _is_valid_adm(iso3, adm):
        raise KeyError(f"ADM level '{adm}' doesn't exist for country '{territory}' ({iso3})")
...
def get_adm(territories: Union[str, List[str]], adm: Union[str, int], simplified=True) -> dict:
    if isinstance(territories, str):
        territories = [territories]
    geojson_features = [geojson.loads(_get_data(i, adm, simplified)) for i in territories]
    feature_collection = {
