...
def aggregate_samples(
    ...
    def process_image(image):
        ...
        samples_per_category = (
            samples_per_image // 2
        )  # Half for flooded, half for unflooded
        ...
        unflooded_samples = sample_conditionally(
            image.updateMask(image.select(flooded_mask_band).eq(0)),
            "unflooded",
            samples_per_category,
        )
        flooded_samples = sample_conditionally(
            image.updateMask(image.select(flooded_mask_band).eq(1)),
            "flooded",
            samples_per_category,
        )
...
def evaluate_classifier(testing_samples, classifier):
    ...
    testAccuracy = testing_samples.classify(classifier).errorMatrix(
...
def export_results_to_cloud_storage(
    accuracyMatrix, dataset_name, bucket_name, filePrefix
):
    ...
    print(
        f"Uploaded combined accuracy and confusion matrix to {bucket_name} with prefix {filePrefix}"
    )
...
def train_and_evaluate_classifier(
    ...
    class_points = [
        int((freq / sum(class_histogram.values())) * samples_per_image)
        for freq in class_histogram.values()
    ]

    all_samples = aggregate_samples(
        image_collection, bbox, class_values, class_points, samples_per_image
    )
    ...
    training_samples, testing_samples, validation_samples = prepare_datasets(
        all_samples
    )
    ...
    export_results_to_cloud_storage(
        testAccuracy, "Testing", bucket_name, testFilePrefix
    )

    valFilePrefix = f"data/{snake_case_place_name}/outputs/validation_results"
    export_results_to_cloud_storage(
        validationAccuracy, "Validation", bucket_name, valFilePrefix
    )
